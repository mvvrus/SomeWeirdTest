...
namespace ScheduleLibrary.Test
{
    ...
    public class ListParserTests
    {
        ...
        public void RangeParser_ParseRangeTest()
        {
            ...
            Assert.IsFalse(RangeParser.RANGE_PARSER.ParseRange(t[1], out RangeStart, out RangeEnd, 1, 26));
            Assert.IsFalse(RangeParser.RANGE_PARSER.ParseRange(t[2], out RangeStart, out RangeEnd, 1, 26));
            Assert.IsFalse(RangeParser.RANGE_PARSER.ParseRange(t[3], out RangeStart, out RangeEnd, 1, 26));
        ...
        public void RangeParser_ParseTest()
        {
            ...
            BoolMap = SaveMap;
            BoolMap.Reset();
            Assert.IsFalse(RangeParser.RANGE_PARSER.Parse(t[1], ref BoolMap, 1, 26));
            BoolMap = SaveMap;
            BoolMap.Reset();
            Assert.IsFalse(RangeParser.RANGE_PARSER.Parse(t[2], ref BoolMap, 1, 26));
            BoolMap = SaveMap;
            BoolMap.Reset();
            Assert.IsFalse(RangeParser.RANGE_PARSER.Parse(t[3], ref BoolMap, 1, 26));
            BoolMap = SaveMap;
            BoolMap.Reset();
        ...
        public void StepwiseParser_ParseTest()
        {
            ...
            BoolMap = SaveMap;
            BoolMap.Reset();
            Assert.IsFalse(StepwiseParser.STEPWISE_PARSER.Parse(t[1], ref BoolMap, 1, 12)); //"-6/4"
            BoolMap = SaveMap;
            BoolMap.Reset();
            Assert.IsFalse(StepwiseParser.STEPWISE_PARSER.Parse(t[2], ref BoolMap, 1, 12)); //"7-/3"
            BoolMap = SaveMap;
            BoolMap.Reset();
        ...
        {
            ListParser parser = new ListParser(1, 12);
            const String test_cases = "5;3,7;1-3,5-8/2,10;*/2,2,3;*;1,*,3,4-6,9-12/2;*,*/2;;1,2,;0,1;2,*/1;3,5 ,4;3,0-2;3,5,13";
            const char case_delim = ';';
            StringPartArray t = new StringPartArray(test_cases.Count(c => c == case_delim)+1);
            bool[] BoolMap = new bool[12];
            bool[] SaveMap = BoolMap;
            ...
            BoolMap = SaveMap;
            BoolMap.Reset();
            Assert.IsFalse(parser.Parse(t[5], ref BoolMap));//"1,*,3,4-6,9-12/2"
            BoolMap = SaveMap;
            BoolMap.Reset();
            Assert.IsFalse(parser.Parse(t[6], ref BoolMap));//"*,*/2"
            BoolMap = SaveMap;
            BoolMap.Reset();
            ...
            BoolMap = SaveMap;
            BoolMap.Reset();
            Assert.IsFalse(parser.Parse(t[11], ref BoolMap)); //"3,5 ,4"
            BoolMap = SaveMap;
            BoolMap.Reset();
            Assert.IsFalse(parser.Parse(t[12], ref BoolMap)); //"3,0-2"
            BoolMap = SaveMap;
            BoolMap.Reset();
            Assert.IsFalse(parser.Parse(t[13], ref BoolMap)); //"3,5,13"
            BoolMap = SaveMap;
            BoolMap.Reset();
        }
